...
class BaseDataset(Dataset):
    ...
    def process_pairs_rev(self, ref_image, ref_mask, tar_image, tar_mask, max_ratio = 0.8):
        assert mask_score(ref_mask) > 0.90
        assert self.check_mask_area(ref_mask) == True
        assert self.check_mask_area(tar_mask)  == True
        ...
        # Filtering background for the reference image
        ref_mask_3 = np.stack([ref_mask,ref_mask,ref_mask],-1)
        masked_ref_image = ref_image * ref_mask_3 + np.ones_like(ref_image) * 255 * (255-ref_mask_3)
        
        rev_masked_ref_image = ref_image * (255 - ref_mask_3) + np.ones_like(ref_image) * 255 * ref_mask_3
